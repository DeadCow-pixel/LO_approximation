...
the QKD literature~\cite{Pirandola:20,opt3040030,Zhang:apr:2024}), to demonstrate its applicability. \textcolor{red}{We find that for the asymmetrical double homodyne POVM an extension to the set of squeezed coherent states is required. The results for asymmetrical measurements are then used to calculate the asymptotic secret fraction for an ideal CV-QKD system to estimate the impact of measurement asymmetry on protocol security.}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Coherent states protocol}
\label{sec:protocol}

In this section, we use our results for asymmetrical measurements to analyze the coherent states protocol used in continuous-variable quantum key distribution (CV-QKD). We will calculate the mutual information, Holevo information, and asymptotic secret fraction of an ideal CV-QKD system in the untrusted noise scenario to evaluate the impact of measurement asymmetry on protocol security.

In a coherent states protocol, Alice prepares an ensemble of coherent states, with amplitudes $\alpha$ distributed according to zero-mean normal distribution, $G(0; \tilde V_{\ind{mod}})$, where $\tilde V_{\ind{mod}}$ is the modulation variance. The distributions
of the quadrature operators $\hat{q}$ and $\hat{p}$ are given by
\begin{align}
\label{eq:p(Vmod)}
    p(\hat{q})=p(\hat{p})\equiv p(\hat{q},\hat{p})=G(0;  V_{\ind{mod}}=4\tilde V_\ind{mod})
\end{align}
where we used the relation $2\hat{a}=\hat{q}+i\hat{p}$, implying $4V(\alpha)\equiv 4\tilde V_{\ind{mod}}=V(\hat{q})=V(\hat{p})\equiv V_{\ind{mod}}$. After transmission through a Gaussian
channel, which simply reduces the coherent amplitude by a factor of $\sqrt{T}$, where $T$ is the channel transmission, $\ket{\alpha}\mapsto\ket{\sqrt{T}\alpha}\equiv\ket{\tilde\alpha}$, the quadratures are measured by Bob. The resulting quadrature distribution is 
\begin{align}
\label{eq:pB}
...
\end{align}
where $\hat{\Pi}$ is the POVM describing the measurements and $Q_{\Pi}(\alpha)$ is its' $Q$-function. Note that the variance $p(\tilde \alpha)$ is $TV_{\ind{mod}}$.

Assuming Bob performs homodyne detection, variance of $p_{\ind{B}}$ in Eq.~\eqref{eq:pB} reads
\begin{equation}
\label{eq:VB_HOM_nonoise}
    V_{\ind{B}}(\hat q, \hat p)\equiv V_{\ind{B}}^{\ind{HOM}}=%{T} V_{\ind{mod}}+\sigma_N+1=
    {T} V_{\ind{mod}}+\sigma_x.
\end{equation}
Analogously, adding any stochastic noise (e.g. channel noise) of variance $\xi$ would modify the variance $V_{\ind{B}}^{\ind{HOM}}$ as follows
\begin{align}
\label{eq:VB_hom}
    V_{\ind{B}}^{\ind{HOM}}={T} V_{\ind{mod}}+\sigma_x+\xi.
\end{align}
From this, the signal-to-noise ratio (SNR) and mutual information between Alice and Bob are
\begin{align}
\label{eq:SNR_hom}
...
\end{align}
hereinafter $\log$ is understood as $\log_2$.

Mathematically, the coherent states protocol is equivalent to Alice generating a two-mode squeezed vacuum state, performing simultaneous measurements of both quadratures on one mode, and sending the other mode through the quantum channel to Bob. The covariance matrix after transmission through a noisy channel is~\cite{laudenbach2018continuous}
\begin{align}
\label{eq:sigma_AB}
    &\Sigma_{\ind{AB}}=
    \begin{pmatrix}
    ...
    \end{pmatrix}
\end{align}
where $V=V_{\ind{mod}}+1$ is the variance of the quadrature operators in shot noise units, $\mathbb{I}=\ind{diag}(1,1)$ and $\sigma_Z=\ind{diag}(1,-1)$.

To calculate Holevo information, we will assume purification attack. In the untrusted noise scenario, asymmetry noise is accessible to Eve, modifying the matrix $\Sigma_{\ind{AB}}$ as $b\mapsto b+\sigma_N= V_{\ind{B}}^{\ind{HOM}}$, i.e.
\begin{align}
\label{eq:Sigma_AB_hom}
    ...
    \end{pmatrix}.
\end{align}
Eve's entropy $S_{\ind{E}}=S_{\ind{AB}}$ is computed from the symplectic eigenvalues of $\Sigma'_{\ind{AB}}$ as~\cite{Weed:rmp:2012}
\begin{align}
\label{eq:nu12}
    &\nu_{1,2} =\frac{1}{2}\left(z\pm\left[V_{\ind{B}}^{\ind{HOM}}-a\right]\right),\notag\\ &z=\sqrt{\left(a+V_{\ind{B}}^{\ind{HOM}}\right)^2-4c^2}.
\end{align}

Bob's partial measurement on his mode, described by the matrix $\Pi_{q,p}$, where $\Pi_q=\ind{diag}(1,0)$ and $\Pi_p=\ind{diag}(0,1)$ are measurements of $\hat{q}$ and $\hat{p}$ quadratures, respectively, transforms Alice's state covariance matrix as~\cite{laudenbach2019analysis}
\begin{align}
    \label{eq:AB}
...
The symplectic eigenvalue $\nu_3$ of $\Sigma_{\ind{A}|\ind{B}}$ is
 ...
 \end{align}
which allows us to calculate the conditional entropy $S_{\ind{E}|\ind{B}}=S_{\ind{A}|\ind{B}}$. Now we are able to calculate
Holevo information
\begin{align}
\label{eq:chi}
...
\label{eq:g(nu)}
    ...
    \frac{\nu-1}{2}\log\frac{\nu-1}{2},
\end{align}
and the asymptotic secret fraction for an ideal CV-QKD system
\begin{equation}
\label{eq:r}
    R=\beta I_{AB}-\chi_{EB},
\end{equation}
where $\beta$ is the  efficiency of information reconciliation.

Assuming that Bob performs double homodyne measurement, analogously to Eq.\eqref{eq:VB_hom}-\eqref{eq:IAB_hom}, we have
\begin{align}
    V_{\ind{B}}^{\ind{DHOM}}&=
...
\end{align}
note that, while dependency on squeezing parameter $r$ is present when separating noise components per Eq.~\eqref{eq:sigma_N-sq-1}, the scenario is only dependent on visible variances $\sigma_{1,2}$, so the form of the double homodyne POVM is irrelevant.

$S_{\ind{E}}$ will be calculated from symplectic eigenvalues of the matrix
\begin{align}
...
Double homodyne detection of Bob's mode will affect the other mode as follows:
\begin{align}
...
\begin{align}
    \nu_3=a-\frac{c^2}{b+1},
\end{align}
now we are able to calculate Holevo information per Eq.~\eqref{eq:chi}.

The effects of asymmetry on mutual information, Holevo information and asymptotic secret fraction are illustrated in Fig.~\ref{fig:homodyne-all} for homodyne detection and in Fig.~\ref{fig:double-homodyne-all} for double homodyne detection. Note that our numerical results for homodyne detection coincide with Ref.~\cite{ruiz2023effects}. Numerical results for asymptotic secret fraction with asymmetrical detection as a function of channel length are shown in Fig.~\ref{fig:r-of-l-all}. It follows that in untrusted noise scenario using asymmetrical homodyne is not beneficial, and double homodyne detection performs better.
...
\caption[]{$R$}
        \end{subfigure}
        \caption{(a) Mutual information (b) Holevo information and (c) asymptotic secret fraction using homodyne measurement as functions of beam splitter transmission
          for various detector efficiencies at $V_{\ind{mod}}=1, T=0.95, \xi=10^{-3}, \beta=0.95$
}
...
\caption[]{$R$}
        \end{subfigure}
        \caption{(a) Mutual information (b) Holevo information and (c) asymptotic secret fraction using double homodyne measurement as functions of signal beam splitter transmission with
        all other beamsplitter taken to be balanced
          for various detector efficiencies (unspecified in legend efficiencies are taken to be unity) at $V_{\ind{mod}}=1, T=0.95, \xi=10^{-3}, \beta=0.95$.
}
...
\label{fig:dhom-r}
\end{subfigure}
\caption{The asymptotic secret fraction as a function channel length computed from for (a) homodyne detection and (b) double homodyne detection
          for various detector efficiencies at $V_{\ind{mod}}=1,T=0.95,\xi=10^{-3}, \beta=0.95$, losses are taken as $20$ dB @ $100$ km.
}
...
\textcolor{blue}{In the main text, we analyzed how measurement asymmetry impacts the performance of CV-QKD system in the untrusted noise scenario, i.e. the asymmetry noise is accessible to Eve. In this case, even relatively small asymmetry significantly reduces maximum channel length (see Fig.~\ref{fig:r-of-l-all}).} \textcolor{red}{In trusted-noise CV-QKD~\cite{usenko2016trusted}, if Alice and Bob are unaware of detector’s arms asymmetry, they would misinterpret the increased variance as channel excess noise rather than a trusted imperfection. This results in an overestimation of the channel’s excess noise and an underestimation of the trusted detector noise. As well as the blinding attack, this strategy can be countered by inserting attenuators to balance the scheme. }

\textcolor{red}{Despite the existence of countermeasures for the attacks described, implementing analytical corrections based on the formalism outlined in this paper—such as modeling asymmetry-induced noise variance and POVM shifts -- would be preferable.}
