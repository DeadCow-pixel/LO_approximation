...
(prepare and measure protocol), denoted by $G(0; V_{\ind{A}})$, where $V_{\ind{A}}$ is the 
...
\begin{align}
  \label{eq:vb}
  V_{\ind{B}} = TV_{\ind{A}}+1+\xi+\sigma_N%=TV+\xi+\sigma_x,
\end{align}
where $\sigma_N$ is defined as per Eq.~\eqref{eq:sgm_N}. The term $1$ corresponds to the vacuum shot noise, modeled as classical noise that adds to the Gaussian 
quadrature variance.
It follows that
\begin{align}
  \label{eq:covqAqB}
  \ind{cov}(A,B)=\sqrt T V_{\ind{A}},
\end{align}
