...
\begin{abstract}
  \textcolor{red}{We study applicability of
...
\label{eq:probabilities}
    p_{\ind{A}}(\alpha)=\frac{1}{\sqrt{\pi V_{\ind{A}}}}\exp\left[
...
\begin{align}
    ...
    \ket{\alpha}\hspace{-2pt}
    \bra{\alpha}.
\end{align}

After transmission through a Gaussian channel, which attenuates the coherent amplitude by a factor of $\sqrt{T}$, where $T$ is the channel transmission, the state is transformed as $\ket{\alpha}\mapsto\ket{\sqrt{T}\alpha}\equiv\ket{\tilde\alpha}$.
The ensemble reads
\begin{align}
\label{eq:rho-tilde}
    \tilde\rho_{\ind{A}}=\frac{1}{T}\int\mathop{\dd^2\tilde\alpha}p_{\ind{A}}(\tilde\alpha)\ket{\tilde\alpha}\hspace{-4pt}\bra{\tilde\alpha}.
\end{align}
...
\begin{align}
\label{eq:pB(x|A)}
    p_{\ind{B}}(x|\tilde\alpha) =
    p_{\ind{A}}(\tilde\alpha) \Tr \left[ \ket{{\tilde\alpha}}\bra{{\tilde\alpha}}\hat{\Pi}_x \right]= 
    p_{\ind{A}}(\tilde\alpha)Q_x(\tilde \alpha),
\end{align}
where $Q_x(\tilde \alpha)$ is the $Q$-function of POVM used. \textcolor{red}{Independent channel noise of variance $\xi$ is modeled as addition to variance of $p_{\ind{A}}(\tilde\alpha)$, i.e. $TV_{\ind{A}}\mapsto TV_{\ind{A}}+\xi$.}

For homodyne detection, Eq.~\eqref{eq:pB(x|A)} takes the form given by  Eq.~\eqref{eq:Pgood-w-def}
\begin{align}
\label{eq:pxa}
   &p_{\ind{B}}^{\ind{H}}(x|\tilde \alpha) = p_{\ind{A}}(\tilde \alpha) Q_x(\tilde \alpha)\notag\\
   &\sim
    \exp\left[
    -\frac{(x-2\tilde q)^2}{2\sigma_x}-\frac{\tilde q^2}{2\left(TV_{\ind{A}}+\xi\right)}
    \right],
\end{align}
where $\tilde q\equiv \sqrt{T}q$. Rewriting exponential's power in Eq.\eqref{eq:pxa} in quadratic form results in
\begin{align}
    ...
    \begin{pmatrix}
        ...
        2{TV_{\ind{A}}}&4TV_{\ind{A}}+\sigma_x
    \end{pmatrix}^{-1}
    \begin{pmatrix}
        \tilde q\\x
    \end{pmatrix}\notag\\&\equiv 
...
\begin{align}
    &p_{\ind{A}}(\tilde \alpha) Q_x(\tilde \alpha)
    \sim
    \exp\biggl[-\frac{(x_1-\tilde q)^2}{\sigma_1}-\frac{\tilde q^2}{2\left(TV_{\ind{A}}+\xi\right)}
    \notag\\&-\frac{(x_2-\tilde p)^2}{\sigma_2}-\frac{\tilde p^2}{2\left(TV_{\ind{A}}+\xi\right)}
    \biggr],
\end{align}
where $\tilde p \equiv\sqrt T p$. Analogously to Eq.~\eqref{eq:sigma-hom-def}, we obtain covariance matrices as follows:
\begin{align}
  \label{eq:Sigma-DH}
    &\Sigma^{\ind{DH}^{(1)}}=\begin{pmatrix}
        {TV_{\ind{A}}}&{TV_{\ind{A}}}\\
        {TV_{\ind{A}}}&{TV_{\ind{A}}}+\frac{\sigma_1}{2}+\xi
    \end{pmatrix},\notag\\
    &\Sigma^{\ind{DH}^{(2)}}=\begin{pmatrix}
        {TV_{\ind{A}}}&{TV_{\ind{A}}}\\
        {TV_{\ind{A}}}&{TV_{\ind{A}}}+\frac{\sigma_2}{2}+\xi
    \end{pmatrix},
\end{align}
...
\end{align}
Substituting Eq.~\eqref{eq:rho_A_th_coh} into Eq.~\eqref{eq:p_A_dh} yields
\begin{align}
...
\end{align}
With scaling by $\lambda$, as well as defining $V=V_{\ind{A}}+1$, the joint probability distribuion in entanglement-based protocol
Eq.~\eqref{eq:pBxa_dh}
...
\begin{align}
  \label{eq:TVMS}
    ...
    \end{pmatrix}
\end{align}
where the identity matrix is $\mathbb{I}=\ind{diag}(1,1)$, and $\sigma_Z=\ind{diag}(1,-1)$, and
\begin{align}
  \label{eq:vb}
  V_{\ind{B}} \equiv 4TV_{\ind{A}}+\xi+\sigma_x.
\end{align}
If homodyne detection is used, Eve's entropy, $S_{\ind{E}}=S_{\ind{AB}}$, is calculated from the symplectic eigenvalues of $\Sigma_{\ind{AB}}^{\ind{TVMS}}$ as~\cite{Weed:rmp:2012}
\begin{align}
\label{eq:nu12}
    &\nu_{1,2}^{\ind{H}} =\frac{1}{2}\left(z\pm\left[V_{\ind{B}}-\alpha\right]\right),\notag\\ &z=\sqrt{\left(\alpha+V_{\ind{B}}\right)^2-4\gamma^2}.
\end{align}
...
\begin{align}
    \label{eq:AB}
    &\Sigma_{\ind{A}|\ind{B}}^{\ind{H}}=\alpha\mathbb{I}-\gamma^2\sigma_Z[\Pi_{q,p}\ b\mathbb{I}\ \Pi_{q,p}]^{-1}\sigma_Z^{\ind T}\notag\\
    &=\alpha\mathbb{I}-\frac{\gamma^2}{b}\Pi_{q,p}.
\end{align}
The corresponding symplectic eigenvalue $\nu_3^{\ind{H}}$ of $\Sigma_{\ind{A}|\ind{B}}$ is
 \begin{align}
     \label{eq:nu3}
     &\nu_3^{\ind{H}}=\sqrt{\alpha\left(\alpha-\frac{\gamma^2}{b}\right)}
 \end{align}
...
\begin{align}
\label{eq:Sigma_AB_dH}
    \Sigma_{\ind{AB}}^{\ind{DH}}=\begin{pmatrix}
        \alpha\mathbb{I}&\gamma\sigma_Z\\
        \gamma\sigma_Z&V_{\ind{B}}^{\ind{DH}}\mathbb{I}
    \end{pmatrix},
\end{align}
where $V_{\ind{B}}^{\ind{DH}}=TV_{\ind{A}}+\xi+\frac{\sigma_1+\sigma_2}{2}$.

Double homodyne detection of Bob’s mode transforms Alice’s conditional covariance matrix as
\begin{align}
    \label{eq:AB_dH}&\Sigma_{\ind{A}|\ind{B}}^{\ind{DH}}=
...
\begin{align}
    \nu_3^{\ind{DH}}=\alpha-\frac{\gamma^2}{b+1}.
\end{align}
...
with Ref.~\cite{ruiz2023effects}, namely the dependence of the asymptotic secret fraction on the beam splitter transmittance deviation. The dependence of the asymptotic secure key rate on channel length in the presence of asymmetrical detection is shown in Fig.~\ref{fig:r-of-l-all}. These results indicate that in the untrusted noise scenario, asymmetric homodyne detection offers no advantage over ideal, symmetric detection, which minimizes the Holevo information. In contrast, double homodyne detection consistently yields better performance.
...
\caption[]{$R$}
        \end{subfigure}
        \caption{(a) Mutual information, (b) Holevo information, and (c) asymptotic secret fraction using homodyne measurement as functions of the beam splitter transmission, for various detector efficiencies at $V_{\ind{A}}=\frac{1}{4}, T=0.95, \xi=10^{-3}, \beta=0.95$
}
...
\caption[]{$R$}
        \end{subfigure}
        \caption{(a) Mutual information, (b) Holevo information, and (c) asymptotic secret fraction using double homodyne detection as functions of the signal beam splitter transmission. All other beam splitters are assumed to be balanced. Results are shown for various detector efficiencies; efficiencies not specified in the legend are taken to be unity. Parameters: $V_{\ind{A}}=\frac14
        , T=0.95, \xi=10^{-3}, \beta=0.95$.
}
...
\label{fig:dhom-r}
\end{subfigure}
\caption{Asymptotic secret fraction as a function of channel length computed for (a) homodyne detection and (b) double homodyne detection, shown for various detector efficiencies. Parameters are $V_{\ind{A}}=\frac14,T=0.95,\xi=10^{-3}, \beta=0.95$. The losses are assumed to be 20 dB per 100 km.
}
