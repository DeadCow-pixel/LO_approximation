...
Note that the condition~\eqref{eq:sigma_N-sq-2} is equivalent to the requirement noise part of POVM covariance matrix $\Sigma^{\ind{DH}}=\ind{diag}(\delta_1,\delta_2)$ being positive, which will become relevant in next section.
...
\end{align} 
where $\Sigma_0^{\ind{(m)}}$ and $\Sigma_N^{\ind{(m)}}$ ($m\in\{\ind{H},\ind{DH}\}$) denote the covariance matrices of the ideal measurement and the associated excess noise, respectively, $\mathbb{I}=\ind{diag}(1,1)$, and $R_\phi$ is the rotation matrix by angle $\phi$.
%$\pi_q=\mathrm{diag}(1,0)$ and $\pi_p=\mathrm{diag}(0,1)$ project onto the corresponding quadratures.
