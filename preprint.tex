...
\begin{align}
\label{eq:rho-tilde}
    &\tilde\rho_{\ind{A}}=\frac{1}{T}\int\mathop{\dd^2\alpha'}\mathop{\dd^2\tilde\alpha}\exp\left[
    -\frac{\left|\tilde\alpha-\alpha'\right|^2}{2\xi}\right]p_{\ind{A}}(\tilde\alpha)\ket{\tilde\alpha}\hspace{-1pt}\bra{\tilde\alpha},
\end{align}
...
\label{eq:pB(x|A)}
    ...
    &\int\mathop{\dd^2\alpha'}\exp\left[
    -\frac{\left|\tilde\alpha-\alpha'\right|^2}{2\xi}\right]p_{\ind{A}}(\tilde\alpha)Q_x(\tilde \alpha),
\end{align}
...
\begin{align}
  \label{eq:dh-exp}
    ...
    &\exp\left[-\frac{(x_1-\tilde q)^2}{2\left(\frac{\sigma_1}2+\xi\right)}-\frac{\tilde q^2}{2TV_{\ind{A}}}
    -\frac{(x_2-\tilde p)^2}{2\left(\frac{\sigma_2}2+\xi\right)}-\frac{\tilde p^2}{2TV_{\ind{A}}}
    \right].
\end{align}
...
\begin{align}
  \label{eq:Sigma-DH}
    &\Sigma^{\ind{DH}^{(i)}}=\begin{pmatrix}
        {TV_{\ind{A}}}&{TV_{\ind{A}}}\\
        {TV_{\ind{A}}}&{TV_{\ind{A}}}+\frac{\sigma_i}{2}+\xi
...
\begin{align}
  \label{eq:I_dh}
    ...
    {\det\Sigma^{\ind{DH}^{(i)}}}\notag\\
    &=\frac{1}{2}\sum_i\log \frac{2TV_{\ind{A}}+\sigma_i+2\xi}
    {\sigma_i+2\xi}.
\end{align}
...
\begin{align}
  ...
  p_{\ind{A}}^{\ind{EB}}(\beta)=\frac{\left(1-\lambda\right)^2}{\pi}\exp\left[\left(\lambda^2-1\right)|\beta|^2\right].
\end{align}
...
Following Ref.~\cite{Serafini:bk:2017}, if covariance matrix of system $AB$ is in the form
\begin{align}
  \label{eq:serf-ab}
    ...
    \end{pmatrix},
  \end{align}
partial measurement of system $B$ transforms $A$ mode as follows
\begin{align}
  \label{eq:serf-a|b}
        \sigma_{A|B}=
...
\sigma_m=SS^{\ind{T}}+Y^*,
\end{align}
where $Y^*$ is proportional to noise. If Bob uses homodyne detection, we have
\begin{align}
    \label{eq:AB}
    &\Sigma_{\ind{A}|\ind{B}}^{\ind{H}}=a\mathbb{I}-c^2\sigma_Z[\Pi_{q,p}\ (V_{\ind{B}}+\sigma_N)\mathbb{I}\ \Pi_{q,p}]^{-1}\sigma_Z^{\ind T}\notag\\
...
where $\Pi_q=\ind{diag}(1,0)$ and $\Pi_p=\ind{diag}(0,1)$, with symplectic eigenvalue $\nu_3^{\ind{H}}$:
 ...
 \end{align}
If double homodyne detection is used, we have
\begin{align}
    \label{eq:AB_dH}&\Sigma_{\ind{A}|\ind{B}}^{\ind{DH}}=
    a\mathbb{I}-c^2\sigma_Z[V_{\ind{B}}\mathbb{I}+\frac{1}{2}\ind{diag}(\sigma_1+1,\sigma_2+1)]^{-1}\sigma_Z^{\ind T}\notag\\&=\begin{pmatrix}
...
\label{fig:dhom-r}
\end{subfigure}
\caption{Asymptotic secret fraction as a function of channel length computed for (a) homodyne detection and (b) double homodyne detection, shown for various detector efficiencies. Parameters are $V_{\ind{A}}=4,T=0.95,\xi=0.5\cdot10^{-3}, \beta=0.95$. The losses are assumed to be 20 dB per 100 km.
}
