...
\begin{align}
  ...
  \alpha={q+ip},
\end{align}
where $q$ and $p$ are independent zero-mean Gaussian random variables with variance $\frac{V_{\ind{A}}}2$, i.e. $q\sim G(q, \frac{V_{\ind{A}}}2)$ and $p\sim G(p, \frac{V_{\ind{A}}}2)$. Therefore, the complex amplitudes $\alpha$
are drawn from a zero-mean Gaussian distribution in the form
%\textcolor{red}{Alice’s preparation of coherent states can be understood as choosing random points in phase space whose coordinates (quadratures) are distributed according to a classical multivariate Gaussian probability distribution {~\cite{Serafini:bk:2017}}:}
...
\begin{align}
\label{eq:pB(x|A)}
    p_{\ind{B}}(x|\alpha) = \int \mathop{\dd^2\tilde\alpha'}
...
\begin{align}
\label{eq:pBxa}
    &p_{\ind{B}}(x, \alpha)=p_{\ind{B}}(x| \alpha)
    p_{\ind{A}}(\alpha),
\end{align}
and marginal distribution
\begin{align}
\label{eq:pBx}
    &p_{\ind{B}}(x)=\int \mathop{\dd^2\alpha} p_{\ind{B}}(x|\tilde \alpha)
    p_{\ind{A}}(\alpha)\end{align}
immediately follow.
...
\begin{align}
  ...
  \begin{pmatrix}
        V_{\ind{A}}&\frac{\sqrt{T}V_{\ind{A}}}2&i\frac{\sqrt{T}V_{\ind{A}}}2\\
        \frac{\sqrt{T}V_{\ind{A}}}2&\frac{TV_{\ind{A}}+\sigma_1}{2}+\xi_1&0\\
        -i\frac{\sqrt{T}V_{\ind{A}}}{2}&0&\frac{TV_{\ind{A}}+\sigma_2}{2}+\xi_2
    \end{pmatrix}
\end{align}
\begin{align}
  I=\frac12\sum\log\frac{2 \left(T V_{A} + 2 \xi_{i} + \sigma_i\right)}{T V_{A} + 4 \xi_{i} + 2\sigma_i}
\end{align}
