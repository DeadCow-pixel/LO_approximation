...
\begin{align}
  \label{eq:dh-exp}
    ...
    -\frac{(x_2-\tilde p)^2}{\left(\frac{\sigma_2}2+\xi\right)}-\frac{\tilde p^2}{2TV_{\ind{A}}}
    \right].
\end{align}
...
\caption[]{}%{$R$}
        ...
        , T=0.95, \xi=2\cdot10^{-3}, \beta=0.95$.
}
...
\label{fig:dhom-r}
\end{subfigure}
\caption{Asymptotic secret fraction as a function of channel length computed for (a) homodyne detection and (b) double homodyne detection, shown for various detector efficiencies. Parameters are $V_{\ind{A}}=\frac14,T=0.95,\xi=2\cdot10^{-3}, \beta=0.95$. The losses are assumed to be 20 dB per 100 km.
}
