...
In this section, we apply our results on asymmetric measurements to analyze the Gausssian-modulated coherent state (GMCS, or GG02~\cite{PhysRevLett.88.057902}) CV-QKD protocol. We compute the mutual information, Holevo information, and asymptotic secret fraction (secure key rate per symbol) under untrusted noise scenario to assess the impact of measurement asymmetry on the protocol’s security.
...
Now we will calculate Holevo information in the untrusted noise scenario, where detection imperfections are modeled as additional noise accessible to the eavesdropper, and, consequently, the noise
contribution is added to the quantum channel parameters, increasing Eve’s accessible information. 
To do so, we will need covariance matrices of POVMs used, which we may deduce from the $Q$-symbols of homodyne~\eqref{eq:Pgood-w-def} and double homodyne~\eqref{eq:P_G-x1x2} detection as:
\begin{align}
  \label{eq:Pi_h}
...
Now we will use the fact that prepare-and-measure (PM) and entanglement-based (EB) protocols are equivalent (see App.~\ref{sec:MI-and-EBtoPm}), allowing us to calculate Holevo information from the two mode squeezed vacuum state (TMSVS) covariance matrix. 
...
(see Ref.~\cite{laudenbach2018continuous}) TMSVS
...
Measurement noise modifies the covariance matrix as
\begin{align}
...
\end{pmatrix},
\end{align}
from symplectic eigenvalues of which we will calculate Eve's entropy, $S_{\ind{E}}=S_{\ind{AB}}$.

To calculate conditional entropy $S_{\ind{E|B}}=S_{\ind{A|B}}$, we will apply partial measurements formula~\cite{Serafini:bk:2017}, which for $\Sigma_{\ind{AB}}^{(m)}$ takes the form
\begin{align}
  \label{eq:serf-a|b}
        \Sigma_{\ind{A|B}}^{(m)}=
        a\mathbb{I}-c^2\sigma_z(V_{\ind{B}}\mathbb{I}+\Pi^{(m)})^{-1}\sigma_z,\quad m\in\left\{\ind{H}, \ind{DH}\right\}.
    \end{align}
For homodyne detection, we arrite at symplectic eigenvalue
\begin{align}
     \label{eq:nu3}
     &\nu_3^{\ind{H}}=\sqrt{a\left(a-\frac{c^2}{V_{\ind{B}}+\sigma_N}\right)},
 \end{align}
and for double homodyne, we have
\begin{align}
    \label{eq:nu3-dh}
...
Note that, in the case of double homodyne detection, $S_{\ind{AB}}$ and thereby $\chi_{\ind{EB}}$ are explicitly dependent on squeezing parameter $r$ that lies in the interval~\eqref{eq:r1-r2}. Moreover, as illustated by Fig~\ref{fig:Entropy}
\begin{figure}
  \centering
  \includegraphics[width=\linewidth, ]{pics\S_AB.pdf}
  \caption{a}\label{fig:Entropy}
\end{figure}


\textcolor{red}{For calculations, we will numerically optimize $r$ to maximize $\chi_{\ind{EB}}$. }

The effects of detection asymmetry on mutual information, Holevo information, and the asymptotic secure key rate are illustrated in Fig.~\ref{fig:homodyne-all} for homodyne detection and Fig.~\ref{fig:double-homodyne-all} for double homodyne detection. \textcolor{red}{Our numerical results for the homodyne case are inconsistent 
...
The dependence of the asymptotic secure key rate on channel length in the presence of asymmetrical detection is shown in Fig.~\ref{fig:r-of-l-all}. These results indicate that in the untrusted noise scenario, asymmetry noise significantly reduces maximal channel length, note that double homodyne performs worse due to dependency on $r$. \textcolor{red}{More refs}
