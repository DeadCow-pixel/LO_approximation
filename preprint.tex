...
However, these analyses remain semiclassical and do not address the modification of the measurement process itself within the quantum-optical formalism. 
...
Next, we compute the Holevo information in the untrusted-noise scenario, in which detection imperfections are modeled as additional noise accessible to the eavesdropper (Eve). Consequently, this noise contribution effectively augments the quantum-channel parameters, increasing Eve’s accessible information.

To do so, we need covariance matrices of POVMs used, which can be deduced from the $Q$-symbols of homodyne~\eqref{eq:Pgood-w-def} and double-homodyne~\eqref{eq:P_G-x1x2} detection as
\begin{align}
  \label{eq:Pi_h}
  &\textcolor{red}{\Pi^{\ind{H}}=\pi_{qp}\left(\cdot\right)\pi_{qp}+\sigma_N\pi_{qp}}\equiv \Pi_0^{\ind{H}}+\Pi_N^{\ind{H}},\\
  \label{eq:Pi_dh}
  &\Pi^{\ind{DH}}=
    \begin{pmatrix}
      e^{2r}&0\\0&e^{-2r}
    ...
    &\begin{pmatrix}
        2\sigma_1-1-e^{2r}&0\\
        0&2\sigma_2-1-e^{-2r}
    \end{pmatrix}\equiv \Pi_0^{\ind{DH}}+\Pi_N^{\ind{DH}}
\end{align} 
% where we defined covariance matrices of ideal measurement $\Pi_0^{\ind{(m)}}$ and noise $\Pi_N^{\ind{(m)}}$, $m\in\{\ind{H},\ind{DH}\}$, and $\pi_q=\ind{diag}(1,0)$ and $\pi_p=\ind{diag}(0,1)$ are projectors onto respective quadratures.
where $\Pi_0^{\ind{(m)}}$ and $\Pi_N^{\ind{(m)}}$ ($m\in\{\ind{H},\ind{DH}\}$) denote the covariance matrices of the ideal measurement and the associated excess noise, respectively, while $\pi_q=\mathrm{diag}(1,0)$ and $\pi_p=\mathrm{diag}(0,1)$ project onto the corresponding quadratures.
...
(see Ref.~\cite{laudenbach2018continuous}), it reads
\begin{align}
  \label{eq:TVMS-final}
...
\Sigma_{\ind{AB}}^{(m)}=\begin{pmatrix}
    a\mathbb{I}&c\sigma_z\\
    c\sigma_z&V_{\ind{B}}\mathbb{I}+\Pi_N^{\ind{(m)}}
\end{pmatrix},
\end{align}
from symplectic eigenvalues of which we can calculate Eve's entropy, $S_{\ind{E}}=S_{\ind{AB}}$.

The conditional entropy $S_{\ind{E|B}}=S_{\ind{A|B}}$ is found via partial measurement~\cite{Serafini:bk:2017}, so $\Sigma_{\ind{AB}}^{(m)}$ takes the following form
\begin{align}
  \label{eq:serf-a|b}
        \Sigma_{A|B}^{(m)}=
        a\mathbb{I}-c^2\sigma_z(V_{\ind{B}}\mathbb{I}+\Pi^{(m)})^{-1}\sigma_z,\quad m\in\left\{\ind{H}, \ind{DH}\right\}.
    \end{align}
...
\textcolor{red}{Note that for double-homodyne detection the Holevo information explicitly depends on the squeezing parameter $r$ within the range~\eqref{eq:r1-r2}, allowing maximization of $\chi_{EB}$.}
% For calculations, we will numerically find $r$ that maximizes Holevo information. More context...}

The influence of detection asymmetry on $I_{AB}$, $\chi_{EB}$, and $R$ is illustrated in Figs.~\ref{fig:homodyne-all} and~\ref{fig:double-homodyne-all}.
Our numerical results for the homodyne case are in qualitative agreement with Refs.~\cite{ruiz2023effects,Bartlett:25}: 
the key rate reaches its maximum for a balanced (symmetric) receiver, corresponding to minimal excess noise.
However, the quantitative behavior differs because our approach embeds asymmetry directly into the POVM, thereby attributing the resulting noise to Bob’s conditional entropy $S_{\ind{A|B}}$, 
whereas Ref.~\cite{ruiz2023effects} treats it as channel noise contributing to $S_{\ind{AB}}$. 
In other words, our formulation interprets asymmetry as an intrinsic measurement imperfection rather than an external eavesdropper-accessible parameter, leading to different scaling of $\chi_{EB}$ and $R$ with beam-splitter imbalance and detector mismatch.
The comparison with the semiclassical analysis of Ref.~\cite{Bartlett:25} confirms that both methods capture the same qualitative trend—performance degradation with increasing asymmetry.


% \textcolor{red}{Our numerical results for the homodyne case are inconsistent 
% with Ref.~\cite{ruiz2023effects}: the dependence of the asymptotic secret fraction on the beam splitter transmittance deviation is maximal for the symmetric case, but our results suggest otherwise. This is due our attribution of asymmetry noise to $S_{\ind{A|B}}$, while in Ref.~\cite{ruiz2023effects} it is atributted to Eve, modifying $S_{\ind{AB}}$. Add York!}

Finally, Fig.~\ref{fig:r-of-l-all} shows the dependence of $R$ on the channel length under asymmetric detection. 
In the untrusted-noise model, asymmetry substantially reduces the achievable transmission distance; the degradation is more pronounced for double-homodyne detection due to its explicit dependence on $r$.
% \textcolor{red}{More refs}
...
\caption[]{}%{$R$}
        \end{subfigure}
        \caption{(a) Mutual information, (b) Holevo information, and (c) asymptotic secret fraction using double homodyne detection as functions of the signal beam splitter transmission under trusted noise scenario. All other beam splitters are assumed to be balanced. Results are shown for various detector efficiencies; efficiencies not specified in the legend are taken to be unity. Parameters: $V_{\ind{A}}=1
...
\label{fig:dhom-r}
\end{subfigure}
\caption{Asymptotic secret fraction as a function of channel length computed for (a) homodyne detection and (b) double homodyne detection, shown for various detector efficiencies under trusted noise scenario. Parameters are $V_{\ind{A}}=1,T=0.95,\xi=0.5\cdot10^{-3}, \beta=0.95$. The losses are assumed to be 20 dB per 100 km.}
...
\textcolor{blue}{In the main text, we analyzed how measurement asymmetry affects the performance of a CV-QKD system under the untrusted-noise scenario, where the asymmetry-induced noise is assumed to be accessible to Eve. In this case, even relatively small levels of asymmetry can noticeably reduce the maximum secure transmission distance (see Fig.~\ref{fig:r-of-l-all}).} 
\textcolor{red}{In the trusted-noise model of CV-QKD~\cite{usenko2016trusted}, if Alice and Bob are unaware of an imbalance between the detector arms, they may misattribute the resulting increase in variance to channel excess noise rather than to a trusted detector imperfection. This misinterpretation leads to an overestimation of channel noise and a corresponding underestimation of the trusted detector contribution. Analogous to the situation in detector-blinding attacks, this vulnerability can be mitigated—at least partially—by inserting calibrated attenuators to rebalance the detection scheme.}
