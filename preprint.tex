...
the positive operator-valued measure (POVM) that takes into account the asymmetry effects. The results are applied to calculate the secure key rate of the GG02 protocol under an untrusted noise model. It is found that asymmetry maximizes security key rate over the balanced case. 
...
The paper is organized as follows. In Sec.~\ref{sec:homodyne}, we obtain the statistical distribution of difference photon counts in Gaussian approximation, from which we obtain the respective POVM. %We thoroughly analyze the dependence of the statistical distribution on the asymmetry parameters.  
In Sec.~\ref{sec:double-homodyne}, we conduct the analysis of the double homodyne scheme analogously to the previous section, finding out that the resulting POVM is not well defined for all asymmetry parameters, requiring generalization. In Sec.~\ref{sec:gen-POVM}, we generalize the double homodyne POVM to the set of squeezed coherent states. In Sec.~\ref{sec:protocol}, we apply our results for homodyne and double homodyne detection to calculate the asymptotic secret fraction for an ideal CV-QKD system.
...
\textcolor{red}{From Fig.~\ref{fig:dist-H} it becomes apparent that perfomance of Gaussian approximation worsens in presence of asymmetry, which we will quantify in Appendix~\ref{sec:appendix_numerical}.}
...
In this section, we apply our results on asymmetric measurements to analyze the Gausssian-modulated coherent state (GMCS, or GG02~\cite{PhysRevLett.88.057902}) CV-QKD protocol. We compute the mutual information, Holevo information, and asymptotic secret fraction (secure key rate per symbol) under \textcolor{red}{both trusted and untrusted} noise scenario, to assess the impact of measurement asymmetry on the protocol’s security.
...
If Bob used double homodyne detection, mutual information takes the form
\begin{align}
  \label{eq:I_DH_res}
  I_{\ind{AB}}^{\ind{DH}}=\frac{1}{2}\sum_i\log\left[1+\frac{2TV_{\ind{A}}}
    {\sigma_i+\xi}\right],
\end{align}
where $\sigma_i$ are defined by Eq.~\eqref{eq:sigm-x-i}/
The details of these derivations are given in Appendix~\ref{sec:MI-and-EBtoPm}.

\textcolor{red}{First, we will calculate Holevo information in the trusted noise scenario. [add desc]} We will use the fact that prepare-and-measure (PM) and entanglement-based (EB) protocols are equivalent (see App.~\ref{sec:MI-and-EBtoPm}), allowing us to calculate Holevo information from the two mode squeezed vacuum state covariance matrix. 
...
(see Ref.~\cite{laudenbach2018continuous})
...
\begin{align}
  \label{eq:TVMS-final}
    ...
    \end{pmatrix},
\end{align}
where $\mathbb{I}=\ind{diag}(1,1), \sigma_Z=\ind{diag}(1,-1)$, and
\begin{align}
  \label{eq:V(V_A)}
    V=1+4V_{\ind{A}}.
\end{align}
...
Eve's entropy, $S_{\ind{E}}=S_{\ind{AB}}$, is calculated from the symplectic eigenvalues of $\Sigma_{\ind{AB}}^{\ind{EB}}$ as~\cite{Weed:rmp:2012}
\begin{align}
\label{eq:nu12}
    &\nu_{1,2} =\frac{1}{2}\left(z\pm\left[V_{\ind{B}}-a\right]\right),\notag\\ &z=\sqrt{\left(a+V_{\ind{B}}\right)^2-4c^2}.
\end{align}

If covariance matrix of system $AB$ is in the form
\begin{align}
  \label{eq:serf-ab}
    ...
    \end{pmatrix},
  \end{align}
noisy Gaussian measurement of system $B$ transforms $A$ mode as follows~\cite{Serafini:bk:2017},
\begin{align}
  \label{eq:serf-a|b}
        \sigma_{A|B}^{(m)}=
        A-C(B+\sigma_\Pi^{(m)})^{-1}C^{\ind{T}},\quad m\in\left\{\ind{H}, \ind{DH}\right\}
    \end{align}
where $\sigma_\Pi^{(m)}$ is the POVM covariance matrix, deduced from Eq.~\eqref{eq:Pgood-w-def} and Eq.~\eqref{eq:P_G-x1x2} for homodyne and double homodyne respectively as
\begin{align}
\label{eq:sigma_m-h}
\sigma_\Pi^{\ind{H}}=\textcolor{red}{\mathbb{I}+\sigma_N\Pi_{qp}},\\
\label{eq:sigma_m-dh}
\sigma_\Pi^{\ind{DH}}=\begin{pmatrix}
        2\sigma_1-1&0\\
        0&2\sigma_2-1
    \end{pmatrix},
\end{align}
where $\Pi_q=\ind{diag}(1,0)$ and $\Pi_p=\ind{diag}(0,1)$.
If Bob uses homodyne detection, we arrive at conditional matrix with symplectic eigenvalue
\begin{align}
    \label{eq:AB}
    &\Sigma_{\ind{A}|\ind{B}}^{\ind{H}}
    =a\mathbb{I}-\frac{c^2}{V_{\ind{B}}+\sigma_N}\Pi_{q,p},\\
     \label{eq:nu3}
     &\nu_3^{\ind{H}}=\sqrt{a\left(a-\frac{c^2}{V_{\ind{B}}+\sigma_N}\right)}.
 \end{align}
If Bob uses double homodyne measurement, we have
\begin{align}
  \label{eq:AB-dh}
  &\Sigma_{\ind{A}|\ind{B}}^{\ind{DH}}=\begin{pmatrix}
        a - \frac{c^{2}}{V_{\ind{B}} + {2\sigma_{1}-1}} & 0\\0 & a - \frac{c^{2}}{V_{\ind{B}} + {2\sigma_{2}-1}}
    \end{pmatrix},\\
    \label{eq:nu3-dh}
    & \nu_3^{\ind{DH}}=a\sqrt{\frac{\left(V_{\ind{B}}  + 2\sigma_1-1 - \frac{c^{2}}a\right) \left( V_{\ind{B}}  + 2\sigma_2-1 - \frac{c^{2}}a\right)}{\left( V_{\ind{B}} + 2\sigma_1-1\right) \left(V_{\ind{B}} + 2\sigma_2-1\right)}}
\end{align}
\textcolor{red}{Note that in this scenario generalization to double homodyne POVM in the form of projectors onto squeezed coherent states Eq.~\eqref{eq:Pi_G-gen-x1x2} from projectors onto coherent states Eq.~\eqref{eq:povm-hetero} is not required as it is not necessery to separate noise part of covariance matrix.}
...
with Ref.~\cite{ruiz2023effects}: the dependence of the asymptotic secret fraction on the beam splitter transmittance deviation is maximal for the symmetric case, but our results suggest otherwise. This is due our attribution of asymmetry noise to $S_{\ind{A|B}}$, while in Ref.~\cite{ruiz2023effects} it is atributted to Eve, modifying $S_{\ind{AB}}$. 

The dependence of the asymptotic secure key rate on channel length in the presence of asymmetrical detection is shown in Fig.~\ref{fig:r-of-l-all}. These results indicate that in the untrusted noise scenario, asymmetric homodyne detection offers slight advantage at large distance over ideal, symmetric detection, which maximizes the Holevo information.  In contrast, double homodyne detection consistently yields better performance. Yet, due to the way $\nu_3^{\ind{DH}}$ is dependent on noise variances, at large distances asymmetry results in lesser secure key rate. At smaller distances, where $T\approx1$, asymmetry reduces the secure key rate of both homodyne and double homodyne detection. \textcolor{red}{LITERATURE WILL BE HERE}

\begin{figure*}
    \centering
    \begin{subfigure}[c]{.3\linewidth}
\includegraphics[width=\linewidth, ]{pics/trust/hom/IAB.pdf}
\caption[]{}%{$I_{\ind{AB}}$}
        \end{subfigure}
\hfill
        \begin{subfigure}[c]{.3\linewidth}
 \includegraphics[width=\linewidth, ]{pics/trust/hom/chi.pdf}
\caption[]{}%{$\chi_{\ind{EB}}$}
\end{subfigure}
\hfill
    \begin{subfigure}[c]{.3\linewidth}
\includegraphics[width=\linewidth, ]{pics/trust/hom/r.pdf}
\caption[]{}%{$R$}
        \end{subfigure}
...
\begin{figure*}
    \centering
    \begin{subfigure}[c]{.3\linewidth}
\includegraphics[width=\linewidth, ]{pics/trust/dhom/IAB.pdf}
\caption[]{}%{$I_{\ind{AB}}$}
        \end{subfigure}
\hfill
        \begin{subfigure}[c]{.3\linewidth}
 \includegraphics[width=\linewidth, ]{pics/trust/dhom/chi.pdf}
\caption[]{}%{$\chi_{\ind{EB}}$}
\end{subfigure}
\hfill
    \begin{subfigure}[c]{.3\linewidth}
\includegraphics[width=\linewidth, ]{pics/trust/dhom/r.pdf}
\caption[]{}%{$R$}
        \end{subfigure}
        ...
        , T=0.95, \xi=0.5\cdot10^{-3}, \beta=0.95$. \textcolor{red}{(Trusted noise)}
}
...
\begin{figure*}
    \centering
    \begin{subfigure}[]{.45\textwidth}
\includegraphics[width=\linewidth]{pics/trust/hom/r(l).pdf}
\caption[]{Homodyne detection}
\label{fig:hom-r}
        \end{subfigure}
        \begin{subfigure}[]{.45\textwidth}
 \includegraphics[width=\linewidth]{pics/trust/dhom/r(l).pdf}
...
\textcolor{red}{(Trusted noise)}
}
\label{fig:r-of-l-all}
\end{figure*}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\textcolor{red}{Now we will move on to untrusted noise scenario [add desc]}. Conditional entropy is calculated from symplectic eigenvalues
\begin{align}
  \label{eq:mu3-h}
  &\mu_3^{\ind{H}}=\sqrt{a\left(a-\frac{c^2}{V_{\ind{B}}}\right)},\\
  \label{eq:mu3-dh}
    &\mu_3^{\ind{DH}}=a-\frac{c^2}{V_{\ind{B}}+1}.
\end{align}
Joint entropy will be calculated from symplectic eigenvalues of the matrix
\begin{align}
    \Sigma^{\ind{EB}'}_{\ind{AB}}\begin{pmatrix}
        a\mathbb{I}&c\sigma_Z\\
        c\sigma_Z&V_{\ind{B}}\mathbb{I}+\sigma_{\eta}^{(m)}
    \end{pmatrix},
\end{align}
where $\sigma_{\eta}^{(m)}$ is the noise part of $\sigma_\Pi^{(m)}$.
Explicitly, per Eq.~\eqref{eq:sigma_m-h} and \eqref{eq:sigma_m-dh},
\begin{align}
    &\sigma_{\eta}^{\ind{H}}=\sigma_N\Pi_{qp},\\
    &\sigma_{\eta}^{\ind{DH}}=\begin{pmatrix}
        2\sigma_1-1-e^{2r}&0\\
        0&2\sigma_2-1-e^{-2r}
    \end{pmatrix}.
\end{align}
Note that Holevo information for double homodyne detection is explicitly dependent on squeezing parameter $r$ that lies in the interval~\eqref{eq:r1-r2}. For calculations, we will numerically find $r$ that maximizes Holevo information. \textcolor{red}{Describing \ref{fig:untrust-homodyne-all}, \ref{fig:untrust-double-homodyne-all}, \ref{fig:untrust-r-of-l-all}.}
...
\caption[]{}%{$R$}
        \end{subfigure}
        \caption{(a) Holevo information and (b) asymptotic secret fraction using homodyne measurement as functions of the beam splitter transmission, for various detector efficiencies at $V_{\ind{A}}=1, T=0.95, \xi=0.5\cdot10^{-3}, \beta=0.95$ \textcolor{red}{(Untrusted noise)}
}
\label{fig:untrust-homodyne-all}
\end{figure*}
...
\caption[]{}%{$R$}
        \end{subfigure}
        \caption{(a) Holevo information and (b) asymptotic secret fraction using double homodyne detection as functions of the signal beam splitter transmission. All other beam splitters are assumed to be balanced. Results are shown for various detector efficiencies; efficiencies not specified in the legend are taken to be unity. Parameters: $V_{\ind{A}}=1
        , T=0.95, \xi=0.5\cdot10^{-3}, \beta=0.95$.
        \textcolor{red}{(Untrusted noise)}
}
\label{fig:untrust-double-homodyne-all}
\end{figure*}
\begin{figure*}
    \centering
    \begin{subfigure}[]{.45\textwidth}
...
\caption[]{Homodyne detection}
\label{fig:untrust-hom-r}
        \end{subfigure}
        \begin{subfigure}[]{.45\textwidth}
 \includegraphics[width=\linewidth]{pics/untrust/dhom/r(l).pdf}
\caption[]{Double homodyne detection}
\label{fig:untrust-dhom-r}
\end{subfigure}
\caption{Asymptotic secret fraction as a function of channel length computed for (a) homodyne detection and (b) double homodyne detection, shown for various detector efficiencies. Parameters are $V_{\ind{A}}=1,T=0.95,\xi=0.5\cdot10^{-3}, \beta=0.95$. The losses are assumed to be 20 dB per 100 km. \textcolor{red}{(Untrusted noise)}
}
\label{fig:untrust-r-of-l-all}
\end{figure*}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
In the GG02 protocol, Alice prepares an ensemble of coherent states $\ket{\alpha=q+ip}$
