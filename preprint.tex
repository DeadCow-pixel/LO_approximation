...
\begin{align}
  \label{eq:Pi_h}
  &\Pi^{\ind{H}}=\pi_{qp}\left(\cdot\right)\pi_{qp}+\sigma_N\pi_{qp}\equiv \Pi_0^{\ind{H}}+\Pi_N^{\ind{H}},\\
  \label{eq:Pi_dh}
  &\Pi^{\ind{DH}}=
    \begin{pmatrix}
      e^{2r}&0\\0&e^{-2r}
...
Using the equivalence between the prepare-and-measure (PM) and entanglement-based (EB) pictures% (see App.~\ref{sec:MI-and-EBtoPm})
, the Holevo information can be derived from the covariance matrix of the two mode squeezed vacuum (TMSV) state $\Sigma^{\ind{EB}}_{\ind{AB}}$. 
...
Note that, in the case of double homodyne detection, $S_{\ind{AB}}$ and thereby $\chi_{\ind{EB}}$ are explicitly dependent on squeezing parameter $r$ that lies in the interval~\eqref{eq:r1-r2}. Moreover, as illustated by Fig~\ref{fig:Entropy}, the choice of $r$ significantly affects the value of $\chi_{\ind{EB}}$. For calculations, we will optimize $r$ to maximize $\chi_{\ind{EB}}$.
\begin{figure}
  \centering
  \includegraphics[width=.75\linewidth, ]{pics/S_AB.pdf}
  \caption{Joint entropy $S_{\ind{AB}}$ as a function of squeezing parameter $r$ for various detector efficiencies with all beam spliiters taken to be balanced.}\label{fig:Entropy}
\end{figure}
