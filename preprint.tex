...
In this section, we apply our results on asymmetric measurements to analyze the Gausssian-modulated coherent state (GMCS, or GG02~\cite{PhysRevLett.88.057902}) CV-QKD protocol. We compute the mutual information, Holevo information, and asymptotic secret fraction (secure key rate per symbol) under an untrusted noise scenario, to assess the impact of measurement asymmetry on the protocol’s security.
...
For homodyne detection, Eq.~\eqref{eq:pB(x|A)} takes the form given by  Eq.~\eqref{eq:Pgood-w-def}
\begin{align}
\label{eq:pxa}
   p_{\ind{B}}^{\ind{H}}(x|\tilde \alpha) 
  \sim
    \exp\left[
    -\frac{(x-2\tilde q)^2}{2\left(\sigma_x+\xi\right)}-\frac{\tilde q^2}{2TV_{\ind{A}}}
    \right].
\end{align}
...
\begin{align}
    ...
    \begin{pmatrix}
        ...
        2{TV_{\ind{A}}}&4TV_{\ind{A}}+\sigma_x+\xi
    \end{pmatrix}^{-1}
    \begin{pmatrix}
        \tilde q\\x
    \end{pmatrix}\notag\\&\equiv 
...
\label{eq:IABPM}
    ...
    {\det\Sigma^{\ind{H}}}=\frac{1}{2}\log \frac{4TV_{\ind{A}}+\sigma_x+\xi}
    {\sigma_x+\xi},
\end{align}
where $\Sigma^{\ind{H}}_{ii}$ are diagonal elements of $\Sigma^{\ind{H}}$, and all logarithms are base 2.

If double homodyne detection is used, from Eq.~\eqref{eq:pB(x|A)} we have (see Eq.~\eqref{eq:P_G-x1x2}):
\begin{align}
  \label{eq:dh-exp}
...
\begin{align}
  \label{eq:I_dh}
    ...
    {\det\Sigma^{\ind{DH}^{(i)}}}\notag\\
    &=\frac{1}{2}\sum_i\log \frac{2TV_{\ind{A}}+\sigma_i+\xi}
    {\sigma_i+\xi}.
\end{align}

To calculate Holevo information, first, consider two mode squeezed vacuum state (TMSVS), in ket notation written as{~\cite{Weed:rmp:2012}}
\begin{align}
  \label{eq:psi_AB}
...
It follows that covariance matrix of TMSVS can be used to calculate Holevo information in the PM protocol. It is defined as follows
\begin{align}
  \label{eq:TVMS}
...
\begin{align}
  \label{eq:TVMS-final}
    &\Sigma^{\ind{EB}}_{\ind{AB}}=\begin{pmatrix}
        ...
        \sqrt{T\left(V^2-1\right)}\sigma_Z&\left[T(V-1)+1+\xi\right]\mathbb{I}
    \end{pmatrix}
    \notag
    \\&\equiv\begin{pmatrix}
...
\begin{align}
  \label{eq:serf-a|b}
        \sigma_{A|B}=
        A-C(B+\sigma_m)^{-1}C^{\ind{T}},
    \end{align}
where, in case of additive noise, $\sigma_m$ is given by
\begin{align}
\label{eq:sigma_m}
\sigma_m=SS^{\ind{T}}+Y^*,
\end{align}
where $SS^{\ind{T}}$ represents the covariance matrix of the ideal pure Gaussian state associated with the measurement, and $Y^*$ is proportional to the the variance of added noise. If Bob uses homodyne detection, we set $Y^*=\sigma_N\mathbb{I}$ (see Eq.~\eqref{eq:sgm_N}), $SS^{\ind{T}}=\lim_{z\to0}R_\phi^{\ind{T}}\ind{diag}(z,z^{-1})R_\phi$, $R_\phi$ is the rotation matrix by angle $\phi$, so we have
\begin{align}
  \label{eq:Rz-lim}
  &\left(V_{\ind{B}}\mathbb{I}
  +\sigma_m
  \right)^{-1}=\notag\\&\lim_{z\to0}\left(V_{\ind{B}}\mathbb{I}
  +R_\phi^{\ind{T}}\ind{diag}(z,z^{-1})R_\phi+\sigma_N\mathbb{I}
  \right)^{-1}=\notag\\
  &\begin{pmatrix}
     \frac{\cos^{2}{\phi}}{V_{B} + \sigma_{N}} & \frac{\sin{2\phi}}{2 \left(V_{B} + \sigma_{N}\right)}\\\frac{\sin2\phi}{2 \left(V_{B} + \sigma_{N}\right)} & \frac{\sin^{2}{\phi}}{V_{B} + \sigma_{N}}, 
  \end{pmatrix},
\end{align}
by setting either $\phi=0$ or $\phi=\frac{\pi}{2}$ to recover $q$ and $p$ quadratures, respectively, and substituting Eq.~\eqref{eq:Rz-lim} back into Eq.~\eqref{eq:serf-a|b}, we arrive at
\begin{align}
    \label{eq:AB}
    &\Sigma_{\ind{A}|\ind{B}}^{\ind{H}}
    =a\mathbb{I}-\frac{c^2}{V_{\ind{B}}+\sigma_N}\Pi_{q,p},
\end{align}
where $\Pi_q=\ind{diag}(1,0)$ and $\Pi_p=\ind{diag}(0,1)$, with symplectic eigenvalue $\nu_3^{\ind{H}}$:
 \begin{align}
     \label{eq:nu3}
     &\nu_3^{\ind{H}}=\sqrt{a\left(a-\frac{c^2}{V_{\ind{B}}+\sigma_N}\right)}.
 \end{align}
If Bob uses double homodyne measurement, $SS^{\ind{T}}=\mathbb{I}$ and $Y^*=\ind{diag}\left(\sigma_N^{(1)},\sigma_N^{(2)}\right)$ (see Eq.~\eqref{eq:sigm-N-i}), we have
\begin{align}
  \label{eq:sigma_m-dh}
  \sigma_m=\mathbb{I}+
    \begin{pmatrix}
       \sigma_N^{(1)}&0\\0&\sigma_N^{(2)}
    \end{pmatrix}=\mathbb{I}+
    \begin{pmatrix}
        \frac{\sigma_1}{2}-\frac{1}{2}&0\\0&\frac{\sigma_2}{2}-\frac{1}{2}
    \end{pmatrix}=\notag\\
    \begin{pmatrix}
        \frac{\sigma_1+1}{2}&0\\0&\frac{\sigma_2+1}{2}
    \end{pmatrix}.
\end{align}
\textcolor{red}{Note that, if we decide to use double homodyne POVM in the form of projectors onto squeezed coherent states Eq.~\eqref{eq:Pi_G-gen-x1x2} instead of projectors onto coherent states Eq.~\eqref{eq:povm-hetero}, we would have, as $SS^T=\ind{diag}\left(e^{-2r}, e^{2r}\right)$, and $2\sigma_N^{(i)}=\sigma_i-\frac{1+e^{\pm2r}}{2}$}
\begin{align}
  \textcolor{red}{\sigma_m=
    \begin{pmatrix}
       e^{-2r}&0\\0&e^{2r}
    \end{pmatrix}+
    \begin{pmatrix}
    ...
    \end{pmatrix}}
\end{align}
Using Eq.~\eqref{eq:serf-a|b}, we obtain
\begin{align}
    \label{eq:AB_dH}&\Sigma_{\ind{A}|\ind{B}}^{\ind{DH}}=\begin{pmatrix}
        a - \frac{c^{2}}{V_{\ind{B}} + \frac{\sigma_{1}+1}2} & 0\\0 & a - \frac{c^{2}}{V_{\ind{B}} + \frac{\sigma_{2}+1}2}
    \end{pmatrix},
\end{align}
resulting in symplectic eigenvalue $\nu_3^{\ind{DH}}$ in the form
\begin{align}
  \label{eq:nu3-dh}
    \nu_3^{\ind{DH}}=a\sqrt{\frac{\left(V_{\ind{B}}  + \frac{\sigma_1+1}{2} - \frac{c^{2}}a\right) \left( V_{\ind{B}}  + \frac{\sigma_2+1}2 - \frac{c^{2}}a\right)}{\left( V_{\ind{B}} + \frac{\sigma_{1}+1}2\right) \left(V_{\ind{B}} + \frac{\sigma_{2}+1}2\right)}}.
\end{align}
...
\begin{figure*}
    \centering
    \begin{subfigure}[c]{.3\linewidth}
\includegraphics[width=\linewidth, ]{pics/qkd/dhom/IAB.pdf}
\caption[]{}%{$I_{\ind{AB}}$}
        \end{subfigure}
\hfill
        \begin{subfigure}[c]{.3\linewidth}
 \includegraphics[width=\linewidth, ]{pics/qkd/dhom/chi.pdf}
\caption[]{}%{$\chi_{\ind{EB}}$}
\end{subfigure}
\hfill
    \begin{subfigure}[c]{.3\linewidth}
\includegraphics[width=\linewidth, ]{pics/qkd/dhom/r.pdf}
\caption[]{}%{$R$}
        \end{subfigure}
        \caption{(a) Mutual information, (b) Holevo information, and (c) asymptotic secret fraction using double homodyne detection as functions of the signal beam splitter transmission. All other beam splitters are assumed to be balanced. Results are shown for various detector efficiencies; efficiencies not specified in the legend are taken to be unity. Parameters: $V_{\ind{A}}=\frac1
