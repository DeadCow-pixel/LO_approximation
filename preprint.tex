...
amplitudes $\alpha$ are drawn from a zero-mean Gaussian distribution 
(prepare and measure protocol), denoted by $G(0;  V)$, where $ V$ is the 
modulation variance. \textcolor{red}{
Alice’s preparation of coherent states can be understood as choosing 
...
After the signal passes through a channel with transmittance $T$ that 
adds excess noise of variance $\xi$, and is subsequently measured using 
asymmetrical homodyne detection that adds variance $\sigma_N$, the 
variance of Bob’s measured quadrature is given by
\begin{align}
  \label{eq:vb}
  V_{\ind{B}} = TV+1+\xi+\sigma_N=TV+\xi+\sigma_x,
\end{align}
where $\sigma_N$ ($\sigma_x$) is defined as per Eq.~\eqref{eq:sgm_N} 
(Eq.~\eqref{eq:sigma-x}). The term $1$ corresponds to the vacuum shot 
noise, modeled here as classical noise that adds to the Gaussian 
quadrature variance.
It follows that
\begin{align}
  \label{eq:covqAqB}
  \ind{cov}(A,B)=\sqrt T V,
\end{align}
so the covariance matrix \textcolor{red}{that describes the joint statistics of Alice’s and Bob’s quadrature measurements in the homodyne detection scenario is}
\begin{align}
  \label{eq:S_qp_hom}
    \Sigma^{\ind{HOM}}=\begin{pmatrix}
        V&\sqrt TV\\
        \sqrt TV&V_{\ind{B}}
    \end{pmatrix}.
\end{align}
...
\begin{align}
\label{eq:IABPM}
    I_{\ind{AB}}^{\ind{HOM}}=\frac{1}{2}\log \frac{VV_{\ind{B}}}
    {\det\Sigma^{\ind{HOM}}}=\frac{1}{2}\log \frac{TV+\sigma_x+\xi}
    {\sigma_x+\xi}.
\end{align}
...
\Sigma_{\ind{AB}}=\begin{pmatrix}
    ...
    ,
\end{align}
note the difference in Bob's variance $b$ -- asymmetry noise would be 
...
\begin{align}
  \label{eq:sigma_ab_dhom}
    \Sigma^{\ind{DHOM}}=
    \tilde\Sigma^{\ind{DHOM}}+ N,
\end{align}
where
\begin{align}
\label{eq:N}
    &N=0\oplus\begin{pmatrix}
        \sigma_N^{(1)}&0\\
        0&\sigma_N^{(2)}
    \end{pmatrix}=\notag\\&0\oplus\begin{pmatrix}
        \frac{\sigma_1}2-\frac{1}{2}&0\\
        0&\frac{\sigma_2}2-\frac{1}{2}
    \end{pmatrix}
\end{align}
which is evident from Eq.~\eqref{eq:P_G-x1x2}. \textcolor{red}{
Note that dependency of noise variance on squeezing parameter $r$ (see Eq.~\eqref{eq:sigma_N-sq-1}) vanishes as it is sufficient to consider the case of projectors onto coherent states (See Eq.~\eqref{eq:sigm-N-i}), because both represent the same variances of the photon count difference distribution.}
...
Calculation of Holevo information can be done assuming Alice and Bob share two-mode squeezed vacuum state, and Eve holds purification of it (entanglement-based protocol), as with appropriate scaling the covariance matrices of prepare-and-measure and entanglement-based protocols are equivalent~\cite{laudenbach2018continuous}. We start from covariance matrix
\begin{align}
  \label{eq:TVMS}
...
\begin{align}
\label{eq:nu12}
    &\nu_{1,2}^{\ind{HOM}} =\frac{1}{2}\left(z\pm\left[V_{\ind{B}}^{\ind{HOM}}-a\right]\right),\notag\\ &z=\sqrt{\left(a+V_{\ind{B}}^{\ind{HOM}}\right)^2-4c^2}.
\end{align}
...
\begin{align}
    \label{eq:AB}
    &\Sigma_{\ind{A}|\ind{B}}^{\ind{HOM}}=a\mathbb{I}-c^2\sigma_Z[\Pi_{q,p}\ b\mathbb{I}\ \Pi_{q,p}]^{-1}\sigma_Z^{\ind T}\notag\\
    &=a\mathbb{I}-\frac{c^2}{b}\Pi_{q,p}.
\end{align}
The corresponding symplectic eigenvalue $\nu_3^{\ind{HOM}}$ of $\Sigma_{\ind{A}|\ind{B}}$ is
 \begin{align}
     \label{eq:nu3}
     &\nu_3^{\ind{HOM}}=\sqrt{a\left(a-\frac{c^2}{b}\right)}
 \end{align}
