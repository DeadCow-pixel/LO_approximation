...
the positive operator-valued measure (POVM) that takes into account the asymmetry effects. The results are applied to calculate the asymptotic secret fraction of the GG02 protocol under an untrusted noise model. It is found that asymmetry is detrimental in this case.
...
\begin{align}
  ...
  \prob(\mu)&=e^{-\eta_1|\alpha_1|^2}e^{-\eta_2|\alpha_2|^2}
    \left(\frac{\eta_1|\alpha_1|^2}{\eta_2|\alpha_2|^2}\right)^{\mu/2}
    \notag
  \\
  &
    \times
...
that the amplitude of the local oscillator, $\left|\alpha_L\right|$, is large
...
\end{align}
Minimizing the variance~\eqref{eq:sigma-x} with respect to the transmittance $C^2$  is straightforward:
\begin{align}
  \label{eq:sigma-x-min}
  &\min_{C^2}\sigma_{x}=\left(
\frac{\sqrt{\eta_1}+\sqrt{\eta_2}}{\eta_1+\eta_2}
  \right)^2\ge 1,\\
  \label{eq:C-min}
  &\argmin_{C^2}\sigma_x=\frac{\sqrt{\eta_1}}{\sqrt{\eta_1}+\sqrt{\eta_2}}.
\end{align}

Our next step is to construct
the positive operator-valued measure
(POVM) based on the Gaussian approximation $\prob_G$.
To this end, we note that the probability~\eqref{eq:Pgood-w-def}
is the expectation value of the POVM in the coherent state
given by
\begin{align}
  \label{eq:PG-as-avr}
    \prob_G=\bra{\alpha}\hat{\Pi}_G\ket{\alpha}.
\end{align}
In the case of
...
\begin{align}
  &
  \label{eq:POVM-P0}
    \hat{\Pi}_G^{(0)}=\frac{1}{\left|\alpha_L\right|}|x,\phi\rangle\langle x,\phi|,
\end{align}
...
\begin{align}
  &
  \label{eq:theform}
  \prob_G(x)=\sqrt{\frac{\sigma_x}{\sigma_G}} \int \mathop{\dd x'} G(x-x';\sigma_N)\prob_G^{(0)}(x'),
  \\
  &
  \label{eq:sgm_N}
...
\label{eq:homodyne-povm}    
    \hat{\Pi}_G&=\frac{1}{(\eta_1+\eta_2)CS\left|\alpha_L\right|}
    \notag
  \\
  &
    \times
    \int \mathop{\dd x'} G\left(x-x'; \sigma_N\right) \ket{x',\phi}\hspace{-1pt}\bra{x',\phi}
\end{align}
...
From Eq.~\eqref{eq:Pgood-w-def} we are also able to construct POVM's~\eqref{eq:homodyne-povm} covariance matrix:
\begin{align}
  \label{eq:Pi_h}
...
\caption{Exact (circle dots) and approximate (solid lines with markers) statistical distributions of photon count difference
  ...
  for different efficiencies  at $\left|\alpha_L\right|=5$
...
$\sigma_G=(\eta_1+\eta_2)\left|\alpha_L\right|^2/2$, and
...
the probability minimum between the central and side peaks
...
\begin{align}
  &
  \label{eq:P0-hetero}
    \prob_{G}^{(0)}\left(x_1,x_2\right)=\frac{\left|\avr{z|\alpha}\right|^2}{\pi \left|\alpha_L\right|^2},\quad
\left|\avr{z|\alpha}\right|^2=e^{-|z-\alpha|^2},
\end{align}
where
\begin{align}
  \label{eq:z}
    z=\left(x_1+ix_2\right) e^{i\phi},\quad     x_i=\frac{\mu_i}{\left|\alpha_L\right|}.
\end{align}
So, the POVM is proportional to a projector onto the coherent state
$\ket{z}\equiv\ket{\left(x_1+ix_2\right)e^{i\phi}}$:
\begin{align}
  \label{eq:POVM-hetero-ideal}
    \hat{\Pi}_G^{(0)}(x_1,x_2)=\frac{1}{\pi \left|\alpha_L\right|^2} \ket{z}\hspace{-1pt}\bra{z}.
\end{align}
For nonideal measurements, the probability~\eqref{eq:P_G-x1x2}
can be expressed
...
\int \mathop{\dd \beta_1}\mathop{\dd \beta_2}
    G\left(x_1-\beta_1;\sigma_N^{(1)}\right)
    \notag
  \\
  &
    \times
    G\left(x_2-\beta_2;\sigma_N^{(2)}\right)
    |\avr{\beta e^{i\phi}|\alpha}|^2,
\end{align}
where $\beta=\beta_1+i\beta_2$, and the excess noise variance
...
and the squeezing parameter, respectively. 

To this end, we consider the case where the squeezing parameter is given by
\begin{align}
  \label{eq:sq-param}
...
\begin{align}
\label{eq:squeezed-Q}
    \left|\bra{\beta,r e^{2 i \phi}}\ket{\alpha}\right|^2&=\frac{1}{\cosh r}
    \exp\Biggl[-
    \frac{e^{-r}}{\cosh r}{}
...
to the case with non-vanishing squeezing parameter.
...
provided the squeezing parameter $r$
is ranged between
...
Sole value of the squeezing parameter corresponds to the limiting case of perfect homodyne measurements with $\sigma_{x}^{(1)}=\sigma_{x}^{(2)}=1$
and $\delta_{1}=\delta_{2}^{-1}=q$. In this case, the squeezing parameter is unambiguously defined
\begin{align}
  &
  \label{eq:r-ideal}
...
\begin{align}
  ...
  \prob_{G}^{(1)}(x_1,x_2)=\frac{\cosh \rho}{2\pi |\alpha_L^{(1)}\alpha_L^{(2)}|}
    \left|\avr{\beta e^{i\phi},\rho e^{2i\phi}|\alpha}\right|^2,
  \\
  &
    \label{eq:beta-ideal}
...
the squeezing parameter is no longer uniquely defined.
So, in the interval~\eqref{eq:r1-r2},
we have the decomposition of
the probability~\eqref{eq:P_G-x1x2}
\begin{align}
  \label{eq:P_G-gen-x1x2-2}
    \prob_G(x_1,x_2)&=\frac{\sqrt{\sigma_1\sigma_2}}{2\pi\sqrt{\sigma_G^{(1)}\sigma_G^{(2)}}}\int \mathop{\dd \beta_1}\mathop{\dd \beta_2}
\notag
  \\
  &
    \times
    G\left(x_1e^{-r}-\beta_1;\sigma_N^{(1)}(r)e^{-2r}\right)
    \notag
  \\
  &
    \times
    G\left(x_2e^{+r}-\beta_2;\sigma_N^{(2)}(r)e^{+2r}\right)
    \notag
  \\
  &
    \times
    \left|\avr{\beta e^{i\phi},r e^{2i\phi}|\alpha}\right|^2, 
\end{align}
that varies with the squeezing parameter $r$.
Similarly,
the corresponding POVM 
\begin{align}
  \label{eq:Pi_G-gen-x1x2}
    \hat{\Pi}_G(x_1,x_2)&=\frac{\sqrt{\sigma_1\sigma_2}}{2\pi\sqrt{\sigma_G^{(1)}\sigma_G^{(2)}}}\int \dd \beta_1\dd \beta_2
    \notag
  \\
  &
    \times
...
Constructing POVM's~\eqref{eq:Pi_G-gen-x1x2} covariance matrix from Eq.~\eqref{eq:P_G-gen-x1x2-2} yields
\begin{align}
  \label{eq:Pi_dh}
  \Sigma^{\ind{DH}}&=
    \begin{pmatrix}
...
with $r\neq0$.
One way to make the Gaussian POVM decomposition unique is to impose additional constraints on the noise variances, which in turn fix the value of the squeezing parameter. 
For example,
the product of noise variances is maximized by the midpoint of the interval:
\begin{align}
  \label{eq:mid-r}
  r_0&\equiv\argmax_r\left[\sigma_N^{(1)}(r) \times \sigma_N^{(2)}(r)\right]\notag\\
  &=\frac14\ln\frac{\delta_1}{\delta_2}=\frac{r_1+r_2}2,
\end{align}
which is easily verified by finding the extremum using definitions from Eq.~\eqref{eq:sigma_N-sq-2}.
%the squeezing parameter would take the minimal (maximal) value: $r=\min\{r_1,r_2\}$ ($r=\max\{r_1,r_2\}$)
...
Using the equivalence between the prepare-and-measure (PM) and entanglement-based (EB) pictures (see App.~\ref{sec:MI-and-EBtoPm}), the Holevo information can be derived from the covariance matrix of the two mode squeezed vacuum state (TMSVS) $\Sigma^{\ind{EB}}_{\ind{AB}}$. 
...
(see Ref.~\cite{laudenbach2018continuous}), TMSVSs covariance matrix reads
\begin{align}
  \label{eq:TVMS-final}
...
Assuming measurement noise as accessible to Eve, her entropy $S_{\ind{E}}=S_{\ind{AB}}$ is evaluated from symplectic eigenvalues of the following covariance matrix:
\begin{align}
    \label{eq:sigma_m_cov}
\Sigma_{\ind{AB}}^{(m)}=\begin{pmatrix}
    V\mathbb{I}&c\sigma_z\\
    c\sigma_z&V_{\ind{B}}\mathbb{I}+\Sigma_N^{(m)}
...
\end{align}
where $\delta_i=2\sigma_i-1$ per Eq.~\eqref{eq:delta_12}.
Assuming purification attack, Holevo information is calculated as
\begin{align}
\label{eq:chi}
    \chi_{\ind{EB}} ^{(m)}&\equiv S_{\ind{E}}-S_{\ind{E}|\ind{B}}=S_{\ind{AB}}-S_{\ind{A}|\ind{B}}\notag\\&=\sum_{i=1,2}g(\nu_i^{(m)})-g(\nu_3^{(m)}),
\end{align}
where
\begin{align}
\label{eq:g(nu)}
...
In the symmetrical but nonideal case (red curve), symmetrical interval $[-r_1,r_1]$ arises, and joint entropy is maximized by the midpoint $r=0$. In this case, choosing the POVM in the form superposition of coherent states~\eqref{eq:povm-hetero} is not only allowed, but optimal.
...
\begin{figure}
  ...
  \caption{Dependence of joint entropy on squeezing parameter $r$ (See Eq.~\eqref{eq:r1-r2}) for different detector efficiencies. Unless specified, all beam spliiters are taken to be balanced. Parameters: $V_{\ind{A}}=1,$ $T=0.95,$ $\xi=0.5\cdot10^{-3}$.}\label{fig:Entropy}
\end{figure}
...
\caption[]{}%{$R$}
        \end{subfigure}
        \caption{Dependence of (a) mutual information, (b) Holevo information, and (c) asymptotic secret fraction on beam splitter transmission $C^2$ if receiver uses homodyne detection, for different detector efficiencies at $V_{\ind{A}}=1,$ $T=0.95,$ $\xi=0.5\cdot10^{-3},$ $\beta=0.95$.
}
...
\caption[]{}%{$R$}
        \end{subfigure}
        \caption{Dependence of (a) mutual information, (b) Holevo information, and (c) asymptotic secret fraction on  signal beam splitter transmission $C_S^2$ (see Fig.~\ref{fig:double-homodyne}) if receiver uses double homodyne detection. All other beam splitters are assumed to be balanced. Results are shown for various detector efficiencies; efficiencies not specified in the legend are taken to be unity. Parameters: $V_{\ind{A}}=1,$ $T=0.95,$ $\xi=0.5\cdot10^{-3},$ $\beta=0.95$.}
...
The effects of detection asymmetry on mutual information, Holevo information, and the asymptotic secret fraction are illustrated in Fig.~\ref{fig:homodyne-all} for homodyne detection and Fig.~\ref{fig:double-homodyne-all} for double homodyne detection.
From Fig.~\ref{fig:homodyne-all}, it is evident that while asymmetry in homodyne detection generally degrades performance, the combination of an unbalanced beamsplitter and unequal detector efficiencies can be beneficial, resulting in maxima of the mutual information and secret fraction with given parameters. Correspondingly, the minima of the Holevo information exhibit similar behavior. % Notably, these functions are symmetric and even with shifting local mininimum (symmetry point).
...
This POVM has been found to be well defined across all  parameter settings of the scheme,
...
%reached when the local oscillator amplitude $\left|\alpha_L\right|$ is in
%the vicinity of unity. At $\left|\alpha_L\right|>1$, the distance rapidly drops with
%the LO amplitude. For example, at $|\alpha|=0.5$,
%the distance falls below $0.05$ when the ratio $\left|\alpha_L\right|/|\alpha|$ exceeds five
...
For this scheme, we have deduced the Gaussian approximation (see Eqs.~\eqref{eq:Gaussian-8p}-\eqref{eq:mu_G-i}) and the corresponding POVM expressed in terms of the incoherent Gaussian superposition of projectors onto coherent states (see Eq.~\eqref{eq:povm-hetero}).
As shown in Fig.~\ref{fig:dh-statistics}, the asymmetry induces effects such as shifts and anisotropy of the distributions in the photocount difference plane. 
However, the applicability of double homodyne POVM in the form ~\eqref{eq:povm-hetero} may be broken provided that the beam splitter for the signal mode is unbalanced. 
To resolve this, an extension to the set of squeezed coherent states is needed (see Eq.~\eqref{eq:Pi_G-gen-x1x2}), leading to explicit dependency of POVM on the squeezing parameter, which lies within the interval~\eqref{eq:r1-r2}, as illustrated in Fig.~\ref{fig:squeezing}. 
This implies that, in general, there are infinitely many POVMs representing a single set of parameters for the double homodyne scheme. Consequently, the squeezing parameter requires an additional constraint to ensure the POVM is well-defined.
This inherent non-uniqueness becomes relevant in untrusted noise security analysis. Specifically, while mutual information between Alice and Bob in Gaussian modulated coherent states CV-QKD protocol is calculated with variances of quadrature distribution~\eqref{eq:P_G-x1x2} (see Appendix~\ref{sec:MI-and-EBtoPm} for explicit calculations), joint entropy, and thus Holevo information, is explicitly dependent on the squeezing parameter (see Fig.~\ref{fig:Entropy}). %, due to the need of separating asymmetry noise from perfect measurement. 
We optimized the squeezing parameter to maximize Holevo information.
...
In this paper we analyzed how measurement asymmetry impacts the performance of CV-QKD system in the untrusted noise scenario, where asymmetry noise is accessible to Eve. In this case even relatively small asymmetry significantly reduces maximum channel length (see Fig.~\ref{fig:r-of-l-all}). In trusted-noise CV-QKD~\cite{usenko2016trusted}, if Alice and Bob are unaware of detector’s arms asymmetry, they may misinterpret the increased variance as channel excess noise rather than trusted detector imperfection. This leads to an overestimation of channel excess noise and an underestimation of trusted detector noise. Similar to blinding attacks~\cite{qin2018homodyne}, this vulnerability can be mitigated by inserting attenuators to balance the detection scheme.
...
\caption{Statistical distance $D_P=\mathtt{D}(\prob,\prob_G)$
  ...
  The amplitudes are
  $|\alpha|=1$ and $\left|\alpha_L\right|=5$.
}
\label{fig:delta-eta}
\end{figure*}
...
\end{align}
both repeated here for ease. We limit our numerical results to the coherent signal state only, except in cases where the curves show sufficiently distinct differences.
...
\begin{align}
  &
    ...
    \frac{1}{2}
    \sum_{\mu=-\infty}^{\infty}
    \left\lvert\prob(\mu) -\prob_G(\mu)\right\rvert.
  \end{align}
...
\begin{equation}
    \left|\alpha_L\right|\geq \frac{1}{\sqrt{2(\eta_1S^2+\eta_2C^2)}}\equiv\alpha_N
    \label{eq:renorm}
  \end{equation}
...
for the signal mode prepared in (a)~the coherent state and
  in (b)~the single photon Fock state at different efficiencies  with $\left|\alpha_L\right|=5$.
    }
...
given the LO amplitude $\left|\alpha_L\right|$,
the distance monotonically increases with $|\alpha|$.
It is shown that,
at $\left|\alpha_L\right|=5$ and $|\alpha|>1$,
...
will be small provided that $\left|\alpha_L\right|$ is large
...
in the weak LO range where $\left|\alpha_L\right|<1$.
By contrast, after the second maximum at $\left|\alpha_L\right|>1$,
the distance falls with the LO amplitude
and it drops below $0.05$ at $\left|\alpha_L\right|>2$.
...
the singular limit,
$\lim_{\left|\alpha_L\right|\to 0}\prob_G(\mu)=\delta(\mu)$. 
...
\begin{figure}[ht!]
    ...
    \caption{Distances, $D_P=D(\prob,\prob_G)$ and $D_S=D(P,{\prob}_G^{(s)})$, 
      ...
      and  ${\prob}_G^{(s)}$ (see Eq.~\eqref{eq:tPG-mu}), as a function of $\left|\alpha_L\right|$
...
Assuming that $CS\ne 0$ and $\left|\alpha_L\right|$ is sufficiently large,
...
\begin{align}
  &
    ...
    \approx
    2CS\sqrt{\eta_1\eta_2}\left|\alpha_L\right|^2,
  \\
  &
    \label{eq:z2-A}
    \ln \left(\frac{{\eta_1}|\alpha_1|^2}{{\eta_2}|\alpha_2|^2}\right)^{\frac{\mu}{2}}\approx
    \frac{\mu}{2}\left(
    \ln\frac{\eta_1S^2}{\eta_2C^2}+\frac{\langle\hat{x}_\phi\rangle}{CS\left|\alpha_L\right|}\right)
    \end{align}
...
\begin{align}
  ...
  CS \left|\alpha_L\right|^2\ln\frac{\eta_1S^2}{\eta_2C^2}+
  \left|\alpha_L\right|\avr{\hat{x}_{\phi}}
  \Bigr],
  \\
  &
      \label{eq:tsgm_G}
...
{\prob}_G^{(s)}(\tilde{x})=\frac{1}{\sqrt{2\pi\tilde{\sigma}_G}}
  ...
  &
    \label{eq:tld-x}
    \tilde{x}=\frac{\mu}{{\sqrt{\eta_1\eta_2}\left|\alpha_L\right|}}-
    CS\left|\alpha_L\right|\ln\frac{\eta_1S^2}{\eta_2C^2},
    \\
  &
    \label{eq:tsgm_x}
...
\begin{align}
\label{eq:tpovm}    
    \hat{\Pi}_G^{(s)}&=\frac{1}{\sqrt{\eta_1\eta_2}\left|\alpha_L\right|}
    \notag
  \\
  &
    \times
...
increases with $\left|\alpha_L\right|$ at sufficiently large LO amplitudes.
...
\begin{align}
  \label{eq:dh-exp}
    ...
    \exp\Biggl[-\frac{(x_1-\tilde q)^2}{2\left(\frac{\sigma_1}2+\frac\xi2\right)}-\frac{\tilde q^2}{2TV_{\ind{A}}}\notag\\
...
Now we move on to the equivalency between Prepare-and-Measure (PM) and Entanglement-Based (EB) schemes.
...
\end{align}
where $\lambda=\tanh r,\ r$ is the squeezing parameter. Let Alice hold the state $\varsigma_{\ind{AB}}=\ket{\Psi}_{\ind{AB}}\bra{\Psi}_{\ind{AB}}$. In the EB protocol, Alice measures one mode using a double measurement, which corresponds to a POVM of coherent state projectors $\left\{\hat\Pi_\beta=\frac{\ket{\beta}\hspace{-1pt}\bra{\beta}}\pi\right\}$, and sends the second mode to Bob. The probability that Alice observes double homodyne outcome $\beta$ is given by the Born rule:
\begin{align}
  \label{eq:p_A_EB}
...
\begin{align}
  ...
  p_{\ind{A}}^{\ind{EB}}(\beta)=\frac{1-\lambda^2}{\pi}\exp\left[\left(\lambda^2-1\right)|\beta|^2\right].
\end{align}
...
%     \ln\frac{\eta_1S^2}{\eta_2C^2}+\frac{\avr{\hat{x}_{\phi}}}{CS\left|\alpha_L\right|}\right)
%     \right]},
% \end{align}
...
%       for different values of $\delta\theta$ at $\left|\alpha_L\right|=5$,
