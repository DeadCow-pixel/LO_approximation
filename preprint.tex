...
the squeezing parameter only in  the limiting case
of perfect homodyne measurements
with $\sigma_{x}^{(1)}=\sigma_{x}^{(2)}=1$
and $\delta_{1}=\delta_{2}^{-1}=q$.
In this case, the squeezing parameter is
...
on the noise variances that would fix the value of the squeezing parameter.
...
In the symmetrical but non-ideal case (red curve), symmetrical interval $[-r_1,r_1]$ arises, and joint entropy is maximized by the midpoint $r=0$. In this case, choosing the POVM in the form superposition of coherent states~\eqref{eq:povm-hetero} is not only allowed, but optimal.
